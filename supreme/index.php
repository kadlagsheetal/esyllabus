...
if (isset($_POST['submit'])) {
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        $sql = "select * from supreme where password='$password' AND supreme_id='$username'";
        $result = $conn->query($sql);
        $row = $result->fetch_assoc();
            if ($result->num_rows == 1) {
                $_SESSION['login_supreme']=$row["supreme_id"];
                header("location: dashboard.php"); // Redirecting To Other Page
            } else {
                $error = "Login ID and Password did not match!";
            }
    }
}
}
        ...
        <div class="loginField">
            <form method="post">
                <table cellspacing="10" align="center">
                    <tr><th><input type="text" name="username" placeholder="Login ID"></th></tr>
